...
dechat_1::dechat_1() : Tool("dechat_1")
{
    ...
    */
    ...
    // hiding options
    if (IOptionsParser *p = graphParser->getParser(STR_KMER_ABUNDANCE_MIN_THRESHOLD))
    {
        p->setVisible(false);
    }
    if (IOptionsParser *p = graphParser->getParser(STR_HISTOGRAM_MAX))
    {
        p->setVisible(false);
    }
    if (IOptionsParser *p = graphParser->getParser(STR_SOLIDITY_KIND))
    {
        p->setVisible(false);
    } // oohh. multi-sample dbg construction someday maybe?
    if (IOptionsParser *p = graphParser->getParser(STR_URI_SOLID_KMERS))
    {
        p->setVisible(false);
    }

    // setting defaults
    if (Option *p = dynamic_cast<Option *>(graphParser->getParser(STR_REPARTITION_TYPE)))
    {
        p->setDefaultValue("1");
    }
    if (Option *p = dynamic_cast<Option *>(graphParser->getParser(STR_MINIMIZER_TYPE)))
    {
        p->setDefaultValue("1");
    }
...
struct Functor
{
    void operator()(dechat_1 *dechat)
    {
        typedef GraphUnitigsTemplate<span> GraphType;
        GraphType graph;

        if (dechat->getInput()->get(STR_URI_INPUT) != 0)
        {
            graph = GraphType::create(dechat->getInput(), false /* do not load unitigs after*/);
        }
        else
        {
            throw OptionFailure(dechat->getParser(), "Specifiy -in");
        }
        ...
        if (delete_h5_file)
        {
            ...
            string input = dechat->getInput()->getStr(STR_URI_INPUT);
            string prefix;
            if (dechat->getInput()->get(STR_URI_OUTPUT))
                prefix = dechat->getInput()->getStr(STR_URI_OUTPUT);
            else
...
void dechat_1::execute()
{

    // std::cout << "dechat 2, version " << VERSION;
#ifdef GIT_SHA1
    ...
    Integer::apply<Functor, dechat_1 *>(kmerSize, this);
}
